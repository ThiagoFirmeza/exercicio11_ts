...
let getMultArrow = (num1: number, num2: number): number => num1 * num2;
...
// USANDO FUNCTION DECLARATION
function getDivDeclaration(a: number = 0, b: number = 0): number { return a / b; }
console.log(getDivDeclaration(10, 5));

// USANDO FUNCTION EXPRESSION
const getDivExpression = function (a: number = 0, b: number = 0): number { return a / b; }
console.log(getDivExpression(20, 5));

// USANDO ARROW FUNCTION
let getDivArrow = (a: number, b: number): number => a / b;
...
let textPrinter = (parameter: string | number = 'Não passado valor!'): void =>
    console.log(`Esta é a ${parameter}ª vez que essa string é exibida.`);

for (let i = 0; i < 7; i++) { textPrinter(i + 1); }
...
let millennialWordsUpperCase: string[] = [];
let arrayPrinter = (array: string): void => console.log(array);

for (let i = 0; i < millennialWords.length; i++) { millennialWordsUpperCase[i] = millennialWords[i].toUpperCase(); }
...
let testCountPositive = (array: number[]): string => {
    let counterPositives: number = 0;
    let arraySepPositives: number[] = [];
...
let getOddNumbers = function (array: number[]): number[] {
    let arrayOdd: number[] = [];
    for (let i = 0; i < array.length; i++) {
...
let convertToString = (value: any): string => String(value);
...
let counterQteChar = (strReceived: string): number => strReceived.length;
...
let builderStrLowerCase = (str: string = ''): string => {
    return str.toLowerCase();
}

let varTest: string = builderStrLowerCase("CHOCOTONE E OVO DE PÁSCOA JUNTOS NO MERCADO EM PLENO FEVEREIRO");
console.log(varTest);
/*
  11
...
let findIndex = (char: string = '', str: string = ''): number | undefined => {
...
let concatStringsFunction = (str1: string = '', str2: string = ''): string => str1.concat(str2);
...
let popadorDeArrays = function (array: any[]): any[] {
    array.pop();
    return array;
}
...
let isNullChecker = (valor: any): boolean => {
    if (valor === null) { return true; }
    else { return false; }
}
let varNuller: any = 'High level';
...
/*
  ...
  - Invoque a função que recebe um callback por parâmetro, passando como  
    ...
    foi exibido.
*/
const impressoraCB = (funcPrinterArrayCB: () => void): void => { funcPrinterArrayCB(); }

let funcPrinterArrayCB = (): void => console.log('Teles');
...
const callBackFunc = (callback: (num: number) => number, valor: number): number => { return callback(valor); }

const triplica = (numero: number): number => numero * 3;
...
const logArrayNumbers = (number: number, index: number, array: number[]): void => {
